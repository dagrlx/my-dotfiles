...
local function getIconForApp(bundleId)
    if not bundleId then return app_icons["Default"] end
    return icon_cache[bundleId] or app_icons[bundleId] or app_icons["Default"]
end
...
local function updateDisplaysInfo(callback)
    sbar.exec(QUERY_DISPLAYS, function(rift_displays)
        if not rift_displays or type(rift_displays) ~= "table" or #rift_displays == 0 then
            print("Error: Could not query rift displays")
        ...
        for i, display in ipairs(rift_displays) do
            ...
            if display.uuid and display.space and display.screen_id and display.name then
                ...
                -- print(string.format("[%d] %s - Space %d (UUID: %s...)",
                --     i, display.name, display.space, display.uuid:sub(1, 8)))
            else
                -- print(string.format("Warning: Display %d has incomplete data, skipping", i))
            end
        end

        if #displays_info == 0 then
            print("Error: No valid displays found")
            if callback then callback(false) end
            return
        end
...
local function updateSpaceIcons(spaceId, workspaceData)
    ...
    if workspaceData.windows and #workspaceData.windows > 0 then
        for _, window in ipairs(workspaceData.windows) do
            local bundleId = window.bundle_id
...
local function createWorkspaceItem(display_index, space_id, workspaceData)
    ...
    if not display_index or not space_id or not workspaceData or not workspaceData.name or not workspaceData.index then
        print(string.format("Error: Cannot create workspace with incomplete data (display=%s, space=%s, ws_name=%s, ws_index=%s)",
            tostring(display_index), tostring(space_id),
            tostring(workspaceData and workspaceData.name),
            tostring(workspaceData and workspaceData.index)))
        return nil
    end
...
local function updateWorkspaceAppearance(spaceId, isActive, hasWindows)
    ...
    local bgBorderColor = isActive and colors.dirty_white or (hasWindows and colors.with_alpha(colors.grey, 0.5) or colors.grey)
...
local function updateWorkspacesForDisplay(display_index, space_id)
    ...
    if not display_index or not space_id then
        print("Error: Invalid display_index or space_id")
        return
    end
    ...
    sbar.exec(command, function(workspacesOutput)
        if not workspacesOutput or type(workspacesOutput) ~= "table" then
            print(string.format("Error: Could not get workspaces for display %d (space %d)",
                display_index, space_id))
            return
        end
        ...
        print(string.format("Updating display %d (space %d): %d workspaces",
            display_index, space_id, #workspacesOutput))
        ...
        for _, workspaceData in ipairs(workspacesOutput) do
            ...
            if not workspaceData.index or not workspaceData.name then
                print(string.format("Warning: Skipping incomplete workspace data (index=%s, name=%s)",
                    tostring(workspaceData.index), tostring(workspaceData.name)))
                goto continue
            end
            ...
            if not spaces[spaceId] then
                ...
                if new_space then
                    ...
                    print(string.format("Warning: Failed to create workspace item for %s", spaceId))
                    goto continue
                end
            end
            ...
            if workspaceData.is_active then
                -- print(string.format("  Display %d - WS %s: ACTIVE (%d windows)",
                    display_index, workspaceData.name, workspaceData.window_count or 0))
            end
        ...
        for spaceId, space in pairs(spaces) do
            if space.display_index == display_index and not updatedSpaces[spaceId] then
                print(string.format("  Removing obsolete: %s", spaceId))
...
local function updateAllDisplays()
    updateDisplaysInfo(function(success)
        if not success then
            print("Failed to update displays info")
            return
        end
...
-- print("=== Initializing Rift workspaces (multi-display) ===")
updateAllDisplays()
...
backup_timer:subscribe("system_woke", function()
    -- print("System woke, updating all displays")
    updateAllDisplays()
end)
